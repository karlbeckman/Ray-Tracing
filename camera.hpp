...
/**
 * @brief A class representing a camera capturing 
 * a scene.
 */
class camera {
public:
   ...
   * 
   * @param world the scene description. 
   */
...
private:
  ...
  void initialize() {
    ...
    auto theta = double_to_radians(vfov);
    // auto h = focal_length * tan(theta/2);
    auto h = focus_distance * tan(theta/2);
    ...
    auto defocus_radius = focus_distance * tan(double_to_radians(defocus_angle / 2));
    defocus_disk_u = u * defocus_radius;
    defocus_disk_v = v * defocus_radius;
  }

  /**
   * @brief Get a randomly sampled camera ray for the pixel at location i, j, originating
   * from the camera defocus disk.
   * 
   * @param i 
   * @param j 
   * @return a randomly sampled ray.
   */
  ...
  /** 
   * @return a random point in the square surrounding a pixel at the origin in 
   * a plane. 
   */
  ...
  /**
   * @brief determine of color of the ray r.
   * 
   * @param r the incoming ray.
   * @param depth the recursive depth.
   * @param world our scene.
  ...
  color ray_color(const ray& r, int depth, const hittable& world) {
    ...
    if (depth == 49 && world.hit(r, interval(0.001, infinity), rec1)) {
      int a = 10;
    }
